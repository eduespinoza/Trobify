...
class MainTrobify : AppCompatActivity(), AdaptadorInmuebleBusqueda.OnItemClickListener {
    ...
    lateinit var filtrosAplicados : FiltrosBusqueda.filtros
    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        if(intent.extras != null){
            ...
            println(filtrosAplicados.numHabitaciones)
            mostrarResultadosFiltros()
        }else { prepararPrimerosResultados()}
        prepararBuscador()
        setListeners()
    }
    ...
    fun cargarInmueblesConFiltros(myCallback : (ArrayList<DataInmueble>) -> Unit){
        ...
        db.collection("inmueblesv2").whereEqualTo("numHabitaciones",filtrosAplicados.numHabitaciones)
            .get().addOnCompleteListener{ task ->
                if(task.isSuccessful){
                    for(ficha in task.result){
                        var pisito = ficha.toObject(DataInmueble::class.java)
